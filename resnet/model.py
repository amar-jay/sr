...
class SRResnetModelv1(nn.Module):
    ...
    def __init__(self,config:SRResnetConfig) -> None:
        ...
        self.conv1 = nn.ConvTranspose2d(config.in_channels, config.hidden_channel, kernel_size=config.kernel_size, stride=config.stride)
...
class SRResnetModel(nn.Module):
    ...
    def __init__(self,config:SRResnetConfig) -> None:
        ...
        self.in_channel = nn.Sequential(
                nn.Conv2d(config.in_channels, config.hidden_channel, kernel_size=config.kernel_size, stride=config.stride, padding=config.padding),
        ...
        for i in range(2, config.num_blocks+2):
            modules += [
                    nn.Conv2d(hidden_channel, hidden_channel*i, kernel_size=config.kernel_size, stride=config.stride),
        ...
        for i in range(config.num_blocks):
            modules += [
                nn.ConvTranspose2d(hidden_channel, hidden_channel//(config.num_blocks+1-i), kernel_size=config.kernel_size, stride=config.stride),
        ...
        self.out_channel = nn.Sequential(
            nn.ConvTranspose2d(config.hidden_channel, config.out_channels, kernel_size=config.kernel_size, stride=config.stride, padding=config.padding),
            nn.BatchNorm2d(config.out_channels),
        )
        self.sr_channel = nn.Sequential(
            nn.ConvTranspose2d(config.out_channels, config.out_channels, kernel_size=config.kernel_size, stride=config.stride, padding=config.padding),
            nn.Tanh()
            )
        self.initialize()
